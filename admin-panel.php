...
if (isset($_POST['app-submit'])) {
    ...
    $appdate1 = DateTime::createFromFormat('Y-m-d', $appdate);

    if ($appdate1 !== false) {
        $appdate1->setTimezone(new DateTimeZone('Asia/Kolkata'));
        $appdate1->setTime(0, 0, 0);
        $appdate = $appdate1->format('Y-m-d');

        if (strtotime($appdate) >= strtotime($cur_date)) {
            if ((strtotime($appdate) == strtotime($cur_date) && strtotime($apptime) > strtotime($cur_time)) || strtotime($appdate) > strtotime($cur_date)) {
                ...
                if (mysqli_num_rows($check_query) == 0) {
                    ...
                    echo "<script>alert('We are sorry to inform you that the doctor is not available at this time or date. Please choose a different time or date!');</script>";
                }
            } else {
...
if (isset($_GET['cancel'])) {
    ...
    $query = mysqli_query($con, "UPDATE appointmenttb SET userStatus='0' WHERE ID = '$appointmentId'");
    if ($query) {
...
if(isset($_GET["generate_bill"])){
  ...
  $content .= '
      <br/>
      <h2 align ="center"> Purva Pet Care Center</h2></br>
...
<html lang="en">
  ...
  <a class="navbar-brand" href="#"><i class="fa fa-user-plus" aria-hidden="true"></i> Purva Pet Care Center </a>
